...
Pitfall.prototype.type = "pitfall";
...
Pitfall.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
};

//Block that signals the end of the level
End.prototype.act = function(step, level) {
	var newPos = this.pos;
	this.status = "win";
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // Player loses
	if (type == "lava" && this.status == null) {
		this.status = "lost";
		this.finishDelay = 1;
		alert("Pro Tip: Don't touch the lava.");
  } else if (type == "pitfall" && this.status == null){
		this.status = "lost";
		this.finishDelay = 0.5;
		alert("Pitfall! Better luck next time!");
  } else if (type == "end") {
		this.status = "won";
		this.finishDelay = 0;
